...
如果你用 Arch Linux，恭喜你，你不用折腾这篇博客下面的命令行，直接用我开发的 rime-ice-installer，输入法自动安装并应用下面的所有配置。
...
rime-ice-installer的功能：
1. 安装 Fcitx 和雾凇输入法
2. 自动配置 X11/Wayland 环境
3. 自动配置万象 AI 大模型
3. 配置一些方便的习惯，默认用逗号、句号翻页
4. 自带一套黑色主题友好的皮肤
5. 最最重要的是， 按 Shift 键换英文布局而不是进入 Fcitx 的英文模式。 Fcitx 的英文模式一旦误触进入，很难自动通过键盘切回，必须鼠标点击托盘才能换回中文输入法
