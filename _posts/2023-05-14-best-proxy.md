...
## 机场的使用方法
在国内， VPN 协议基本上全军覆没， 因为 GFW 根据 VPN 协议和 VPN 供应商的 IP 地址就很容易进行封锁， 虽然 VPS 方便安全， 但是 VPS 有一个问题是， 如果目标网站封锁了购买 VPS 的 IP 地址， 每次都要找供应商更换 IP 也很麻烦。

所以， 目前我的方案是， 日常购买一台 VPS 用于上网和测试远程代码。 如果遇到封锁 IP 的情况就临时找一个机场来解决， 机场本质就是在供应商服务器上已经搭建好翻墙方案（比如 v2ray, trojan 等）， 本地安装好 clash 后， 只需要下载机场供应商的 config.yaml 配置文件即可快速联网。

机场安装方法如下：
1. 下载 clash: ```wget https://github.com/Dreamacro/clash/releases/download/v1.14.0/clash-linux-amd64-v1.14.0.gz```
2. 解压 clash: ```gzip -f clash.gz -d```
3. 给执行权限: ```chmod u+x clash```
4. 第一次启动： 先启动 clash 一次， 生成配置文件 ```~/.config/clash/config.yaml``` 后， 按 Ctrl + C 终止
5. 下载供应商配置文件： ```wget -U "Mozilla/6.0" -O ~/.config/clash/config.yaml clash-url```, 其中 clash-url 是机场供应商的 Clash 订阅链接
6. 执行 clash: ```./clash```

Clash 启动以后， 会在本地暴露两个通讯端口， 分别是 HTTP 的 7890 和 SOCKS5 的 7891， 在浏览器 SwitchyOmega 中添加对应端口的规则即可。
