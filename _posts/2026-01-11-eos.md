...
EndeavourOS 的镜像源经常会挂，挂了你就用 eos-rankmirrors 测速后，只保留 ```/etc/pacman.d/endeavouros-mirrorlist``` 中第一个镜像源就好了
...
注意，设置窗口规则的时候，选择 “初试时应用”， 尽量不要选择 “强制”， 比如像 lazycat-terminal 代码还没有写的那么健壮就会出问题。窗口规则中的窗口类型只勾选 "正常窗口", 避免微信窗口的菜单和输入法产生坐标偏移.
...
### 安装 Flameshot
Flameshot是Linux下非常优秀的截图工具条, 非常强大, 但是最新的V14版本无法支持我最喜欢的跨屏幕截图功能, 使用下面的命令安装 V13.3 的版本

```
sudo pacman -Syu
sudo pacman -S --needed qt6-svg kguiaddons hicolor-icon-theme qt6-imageformats
sudo pacman -U https://archive.archlinux.org/packages/f/flameshot/flameshot-13.3.0-2-x86_64.pkg.tar.zst
```
...
在 [options] 下面加一行，或修改已有的 IgnorePkg：
...
* CC Switch: ```yay -S cc-switch-bin``` , CC Switch 是管理各种 AI 中转站的神器
...
1. 第一步，注册火山引擎，然后通过实名认证， https://www.volcengine.com/ 
2. 第二步，在快捷导航里面添加豆包语音
3. 第三步，在这个页面切换为旧版模式，并且创建一个新的应用， https://console.volcengine.com/home
4. 第四步，创建应用的时候，名称、简介随便写，接入能力那边把下面的几个全部勾选
5. 第五步，点击左侧 API 服务中心里面的豆包流式语音识别模型2.0，里面有 APP ID 和访问 Token，复制到 Just Talk 底部的连接配置
...
### SDDM 配置
如果你的 SDDM 登录的时候无法输入密码，可能是 SDDM 比 udev （控制输入设备的服务）还早，导致键盘无法输入，可以用下面的办法修复

创建文件 /etc/systemd/system/sddm.service.d/override.conf
