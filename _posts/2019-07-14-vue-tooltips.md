...
### Vue.js是计算机发展的趋势使然
计算机的历史总是遵循分久必合合久必分的趋势，原来Web前端硬件和浏览器的性能不够，为了用户体验和性能，开发者会选择像 Rails 那样进行服务端模板渲染以提升性能，但是服务端模板渲染的技术弊端是，当前端界面复杂度达到PC软件的规模时，界面的调整太依赖于后端和DOM结构树的查询节点树。

最终的问题是，一旦要切换新页面时就需要后端改动一下，前端也改动一下，一旦前端界面大改时，JQuery这种依赖模板和DOM查询节点的方法就会很被动，因为界面布局和结构发生巨变时，依赖DOM节点查询的方式就很痛苦，项目也很难以维护。
...
原来学Rails的时候，对Vue.js和React的技术抱有偏见，很多Web的开发者总是争论是后端服务渲染好，还是Vue.js这种前端框架好？在我看来，任何撇开时代背景和用户需求的技术讨论都是个人喜好性质的讨论。
...
所以Vue.js这种前端框架的大行其道，并不是技术框架在技术层面的谁优谁劣，而是Web终端硬件性能提升和用户对Web应用功能期待的一种自然而然的演化行为，虽然Vue.js/React这种技术有很多技术细节和优点，但是从架构设计上来看，其实当今的Web技术又回到微软时代的那种前后端分离的技术，架构上是没有区别的，只是Vue.js相对于MFC、Gtk这类的技术，通过属性绑定和动态DOM的技术更有生产力的编写前端界面。

### Vue.js 的优点
对于我这种非常熟悉Gtk/Qt的开发者来说，我更喜欢Vue.js这种前后端分离的设计，因为前端的修改非常有效率，实时可以看到最新的界面效果，同时后端也可以做的很薄，可以让后端专注于架构设计、工程化运维和性能提升。

Vue.js 的优点主要有以下几点：

1. 通过属性绑定和DOM Tree Diff更新的技术来实时渲染Web界面，前端开发生产力非常高
2. Vue.js的作者是国人，中文文档写的非常好，我想这也是Vue.js在中国非常流行的原因，官方的文档看一天即可入门写代码了
3. 内置 VueRouter、Vuex全家桶，包括 vue-cli 的发布，真正做到开箱即用，不用折腾各种插件和让人绝望的Webpack配置即可快速把精力投入到产品开啊中
...
#### Vue.js路由的支持
Vue.js官方文档可能从入门的角度看，都是用单文件的方式来展示Vue.js的示例，但是真的项目往往都是需要前端路由来控制，即使看完Vue.js/Vuex的文档，我相信大多数人还是不知道怎么配置一个 vue-cli 项目去支持前端页面的路由跳转。
...
初学者一般看到这几个关键字都是一脸懵逼的，其实 Vue.js 是对状态的不同用法来进行关键字和区域划分的：
...
1. 首先需要在 Background.js 模块中增加 ```namespaced: true``` 的关键字以增加命名空间
2. 其次，需要在组件中用 ```Background/zoomRatio``` 的前缀去调用 Store 状态。

这样才能在组件中对Store模块的状态进行正常访问

### EventBus 回调调用两次的问题
对于单向的组件之间通讯，比如子组件发送信号给父组件，有时候用 EventBus 更轻便。
...
这个地方的坑在于，一旦页面切换时，页面加载一次就会调用一次 ```EventBus.$on("signal", this.callback)``` , 页面加载多次就会导致调用多次事件回调，然后再项目实践中产生各种诡异的问题。
...
### 最后
虽然上面写了这么多，但是从整体上看，一旦你熟悉了Vue.js的使用方式，Vue.js的坑已经算同等开源项目里面比较少的项目，用起来还是非常省心和好用的。

上面就是我看了官方文档后，在Vue.js项目实践中的一些技巧分享，希望看到此文章的同学少走一些弯路。
